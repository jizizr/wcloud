...
impl<'a> Tokenizer {
    ...
    fn get_word_frequencies(&'a self, text: &'a str) -> HashMap<&'a str, usize> {
    ...
    pub fn get_normalized_word_frequencies(&'a self, text: &'a str) -> Vec<(&'a str, f32)> {
        ...
        self.get_normalized_word_frequencies_from_map(frequencies)
    }
    ...
    pub fn get_normalized_word_frequencies_from_map(
        ...
        let max_freq = *frequencies.values().max().expect("Can't get max frequency");
        let mut normalized_freqs: Vec<(&str, f32)> = frequencies
            .iter()
            .map(|(key, val)| (*key, *val as f32 / max_freq as f32))
            .collect();
...
mod tests {
    ...
    fn filter_works() {
        ...
        println!("original words: {:?} changed: {:?}", words, frequencies);
